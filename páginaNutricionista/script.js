document.addEventListener('DOMContentLoaded', function() {
                    ...
                    <div class="account-container">
                        ...
                        <form id="nutriAccountForm" class="account-form">
                            <div class="form-group">
                                ...
                                <input type="email" id="nutriEmail" required>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="password" id="newPasswordNutri" placeholder="Digite a nova senha (mínimo 6 caracteres)">
                            </div>
                            <div class="form-group">
            ...
            firebase.firestore().collection('pacientes')
                ...
                .then(querySnapshot => {
                    ...
                    if (querySnapshot.empty) {
                        resultadosDiv.innerHTML = '<div class="resultado-paciente">Nenhum paciente encontrado</div>';
                    ...
                    querySnapshot.forEach(doc => {
                        ...
                        resultado.className = 'resultado-paciente';
                        resultado.innerHTML = `<div class="paciente-nome">${paciente.nome || 'Nome não informado'}</div>
                                               <div class="paciente-email">${paciente.email || ''}</div>`;
                        resultado.addEventListener('click', () => {
                            inputPesquisa.value = paciente.nome;
                            pacienteIdInput.value = doc.id;
                            resultadosDiv.style.display = 'none';
                        });
        ...
        firebase.firestore().collection('pacientes').get()
            .then(querySnapshot => {
                ...
                querySnapshot.forEach(doc => {
                    ...
                    patientCard.innerHTML = `<div class="patient-name">${paciente.nome || 'Nome não informado'}</div>
                                             <div class="patient-email">${paciente.email || ''}</div>`;
                    patientCard.addEventListener('click', () => mostrarDetalhesPaciente(doc.id, paciente));
    ...
    function mostrarDetalhesPaciente(pacienteId, paciente) {
        ...
        patientInfo.innerHTML = `<div class="form-group"><label>Nome completo</label><input type="text" value="${paciente.nome || ''}" readonly></div>
                                 <div class="form-group"><label>E-mail</label><input type="email" value="${paciente.email || ''}" readonly></div>
                                 <div class="form-group"><label>Telefone</label><input type="tel" value="${paciente.telefone || ''}" readonly></div>
                                 <div class="form-group"><label>Data de Nascimento</label><input type="text" value="${paciente.dataNascimento || ''}" readonly></div>`;
    ...
    function enviarArquivo(pacienteId, file) {
        alert(`Arquivo "${file.name}" enviado para o paciente ${pacienteId}`);
    }

    function carregarAgenda() {
        const agendaContainer = document.getElementById('agendaContainer');
        agendaContainer.innerHTML = '<p>Carregando agenda...</p>';

        firebase.firestore().collection('consultas')
            .where('nutricionistaId', '==', firebase.auth().currentUser.uid)
            .orderBy('data')
            .orderBy('horario')
            .get()
            .then(querySnapshot => {
                ...
                if (querySnapshot.empty) {
                    agendaContainer.innerHTML = '<p>Nenhuma consulta agendada.</p>';
                    return;
                }
                ...
                querySnapshot.forEach(doc => {
                    ...
                    consultasPorDia[consulta.data].push({...consulta, id: doc.id});
                });

                for (const data in consultasPorDia) {
                    const [ano, mes, dia] = data.split('-');
                    const dayCard = document.createElement('div');
                    dayCard.className = 'day-card';

                    let appointmentsHTML = "";
                    consultasPorDia[data].forEach(consulta => {
                        appointmentsHTML += `<div class="appointment-item">
                                                <span>${consulta.horario} - ${consulta.pacienteNome}</span>
                                                <button class="btn-cancelar" data-id="${consulta.id}">Cancelar</button>
                                             </div>`;
                    });

                    dayCard.innerHTML = `<div class="day-title">${dia}/${mes}/${ano}</div>${appointmentsHTML}`;
                    agendaContainer.appendChild(dayCard);
                }

                document.querySelectorAll('.btn-cancelar').forEach(btn => {
                    btn.addEventListener('click', function() {
                        if (confirm('Tem certeza que deseja cancelar esta consulta?')) {
                            firebase.firestore().collection('consultas').doc(this.getAttribute('data-id')).delete()
                                .then(() => {
                                    showMessage('Consulta cancelada com sucesso', 'success');
                                    carregarAgenda();
                                })
                                .catch(error => {
                                    showMessage('Erro ao cancelar consulta: ' + error.message, 'error');
                                });
                        }
                    });
                });
            ...
            .catch(error => {
                agendaContainer.innerHTML = `<p class="message error">Erro ao carregar agenda: ${error.message}</p>`;
            });
    }
    ...
    function carregarDadosNutricionista() {
        ...
        if (user) {
            document.getElementById('nutriEmail').value = user.email || '';
        }
    }
    ...
    //  Carrega a seção salva ou inicio
